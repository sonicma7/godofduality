...
public class CharacterMotor : MonoBehaviour
{
    ...
    public class CharacterMotorJumping
    {
		...
		public float baseWallHeight = 2.0f;
    ...
    private Vector3 ApplyGravityAndJumping(Vector3 velocity)
    {
        ...
        if(grounded)
		{
			...
			velocity.y = Mathf.Min(0, velocity.y) - movement.gravity * Time.deltaTime;
			velocity.y = Mathf.Max(velocity.y, -movement.maxFallSpeed / 2.0f);
		}
        else
        {
        ...
        if(grounded || ( jumping.jumpCnt < 1 && jumping.letGoJump ) )
        {
			...
			if( jumping.letGoJump && inputJump )
			{
				jumping.letGoJump = false;
				jumping.jumpCnt++;
				jumping.lastButtonDownTime = Time.time;
			}
            ...
            if(jumping.enabled && canControl && (Time.time - jumping.lastButtonDownTime < 0.2))
            {
				...
				if( jumping.onWall ){
					velocity += jumping.jumpDir * CalculateJumpVerticalSpeed(jumping.baseWallHeight) * 2;
					velocity += jumping.jumpBck * jumping.baseWallHeight;
					jumping.onWall = false;
					movement.gravity = movement.gravity * 2.0f;	
				}
				else{
    ...
    void OnControllerColliderHit(ControllerColliderHit hit)
    {
		...
		if( hit.gameObject.tag == "Wall" && jumping.onWall == false ){
			jumping.jumpBck = hit.normal;
			jumping.onWall = true;
			movement.velocity = Vector3.zero;
			movement.gravity = movement.gravity / 2.0f;
			jumping.jumpCnt = 0;
		}
		if( hit.gameObject.tag == "Floor" && jumping.onWall == true ){
			jumping.onWall = false;
			movement.gravity = movement.gravity * 2.0f;
			jumping.jumpCnt = 0;
		}
